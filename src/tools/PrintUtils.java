...
import struct.Node;
	...
	public class PrintUtils {
		...
		public static void toFile(){
			try {
				if(Config.verbose >= 1){
					System.out.println("Generating File : " + Config.out+"."+Config.format);
				}
				BufferedWriter writer = new BufferedWriter(new FileWriter(Config.out+"."+Config.format));		
		...
		private static void dotGenerator(){
			try {
				...
				if(!canDo){
					System.err.println("dot is not installed on this machine will not be able to generate png file");
				}else{
					if(Config.verbose >= 1){
						System.out.print("Generating Trees in format png......");
					}
						pr = Runtime.getRuntime().exec("dot -Tpng -o " + Config.out+".png " + Config.out+"."+Config.format);
					try{
						pr.waitFor();
